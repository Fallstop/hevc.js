...
TEST(IncrementalDecode, FeedDrainMatchesBatch) {
    ...
    for (size_t i = 0; i < min_count; i++) {
        ...
        ASSERT_EQ(bp->plane_samples(0), ip->plane_samples(0))
            << "Y plane size mismatch at frame " << i;
        bool y_match = (bp->plane_bytes[0] == ip->plane_bytes[0]);
        bool cb_match = (bp->plane_bytes[1] == ip->plane_bytes[1]);
        bool cr_match = (bp->plane_bytes[2] == ip->plane_bytes[2]);
        ...
        if (!y_match) {
            ...
            const uint16_t* bpY = bp->plane_ptr<uint16_t>(0);
            const uint16_t* ipY = ip->plane_ptr<uint16_t>(0);
            for (size_t j = 0; j < bp->plane_samples(0); j++) {
                if (bpY[j] != ipY[j]) {
